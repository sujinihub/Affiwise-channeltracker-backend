...
if (!BOT_ADMIN_ID)                 { console.error('❌  BOT_ADMIN_ID missing'); process.exit(1); }
if (!IS_MOCK && !MONGODB_URI)      { console.error('❌  MONGODB_URI missing');  process.exit(1); }

if (IS_MOCK) console.log('🎭  MOCK_DATA=true — running with fake data, no DB required');
...
const channelSchema = new mongoose.Schema({
  ...
  status:      { type: String, default: 'pending', enum: ['pending','active','declined'] },
  addedAt:     { type: Date,   default: Date.now },
...
function generateMockSubscribers(count = 180) {
  const firstNames = ['Ahmed','Lena','Carlos','Yuki','Fatima','Arjun','Sofia','Kwame','Mia','Dmitri','Amara','Lucas','Zara','Hiro','Elena','Omar','Priya','Marco','Aisha','Felix','Nadia','Raj','Ingrid','Kofi','Sara','Bruno','Mei','Aleksei','Layla','Tobias'];
  const lastNames  = ['Hassan','Mueller','Lopez','Tanaka','Al-Rashid','Patel','Rossi','Asante','Chen','Volkov','Diallo','Silva','Ahmed','Nakamura','Popescu','Khalil','Sharma','Bianchi','Ibrahim','Wagner'];
  const usernames  = ['dev_','code_','tech_','pro_','real_','the_','mr_','mz_','just_','its_'];
  const now = Date.now();
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      username:   Math.random() > 0.3 ? (usernames[i % usernames.length] + fn.toLowerCase() + (i % 99)) : null,
...
function computeMockStats(subs) {
  ...
  const weekCount   = subs.filter(s => new Date(s.joinedAt) >= new Date(now - 7*86400000)).length;
  const monthCount  = subs.filter(s => new Date(s.joinedAt) >= new Date(now - 30*86400000)).length;
  const buckets = {};
  subs.forEach(s => {
    const d = new Date(s.joinedAt);
    const key = `${d.getFullYear()}-${d.getMonth()+1}`;
    if (!buckets[key]) buckets[key] = { _id: { year: d.getFullYear(), month: d.getMonth()+1 }, count: 0 };
    buckets[key].count++;
  });
...
async function ensureBootstrapAdmin() {
  ...
  if (!exists) {
    await Admin.create({ telegramId: id, username: 'bootstrap', firstName: 'Owner', addedBy: null });
    console.log(`👑  Bootstrap admin seeded: ${id}`);
  }
}

async function isAdmin(telegramId) {
  if (IS_MOCK) return telegramId === Number(BOT_ADMIN_ID);
...
app.get('/api/subscribers', async (_req, res) => {
  try {
    if (IS_MOCK) return res.json({ success: true, data: generateMockSubscribers(180), mock: true });
...
app.get('/api/stats', async (_req, res) => {
  try {
    if (IS_MOCK) {
      ...
      return res.json({ success: true, data: computeMockStats(subs), mock: true });
    }
    const total      = await Subscriber.countDocuments();
    const today      = new Date(); today.setHours(0,0,0,0);
    const todayCount = await Subscriber.countDocuments({ joinedAt: { $gte: today } });
    const weekCount  = await Subscriber.countDocuments({ joinedAt: { $gte: new Date(Date.now()-7*86400000) } });
    const monthCount = await Subscriber.countDocuments({ joinedAt: { $gte: new Date(Date.now()-30*86400000) } });
    const growth = await Subscriber.aggregate([
      { $group: { _id: { year:{$year:'$joinedAt'}, month:{$month:'$joinedAt'} }, count:{$sum:1} } },
      { $sort: { '_id.year':1, '_id.month':1 } },
      { $limit: 12 },
    ]);
    res.json({ success: true, data: { total, todayCount, weekCount, monthCount, growth } });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
async function adminOnly(ctx, next) {
  if (!(await isAdmin(ctx.from?.id))) return ctx.reply('⛔ You are not authorized.');
  return next();
}

// ── Channel name helper ──
function channelLabel(ch) {
  return ch.username ? `${ch.title} (@${ch.username})` : `${ch.title} [${ch.chatId}]`;
}
  ...
  return {
    ...
    opts: {
      ...
      ...Markup.inlineKeyboard([
        ...
        [Markup.button.callback('📈 Quick Stats', 'stats'), Markup.button.callback('📡 Channels', 'channels')],
...
bot.action('channels_list', adminOnly, async ctx => {
  ...
  if (IS_MOCK) {
    return ctx.editMessageText('📋 *All Channels*\n\n_Mock mode — no channels in DB._', {
      parse_mode: 'Markdown',
  ...
  if (!channels.length) {
    return ctx.editMessageText(
      ...
      { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[backBtn('← Back', 'channels')]]) }
    );
  }
...
bot.action('channels_pending', adminOnly, async ctx => {
  ...
  if (IS_MOCK) {
    return ctx.editMessageText('⏳ *Pending Approval*\n\n_Mock mode — no pending channels._', {
      parse_mode: 'Markdown',
      ...Markup.inlineKeyboard([[backBtn('← Back', 'channels')]]),
    });
  }

  const pending = await Channel.find({ status: 'pending' }).sort({ addedAt: -1 });
  if (!pending.length) {
    return ctx.editMessageText(
  ...
  const buttons = pending.map(ch => [
    Markup.button.callback(`⏳ ${ch.title.slice(0,30)}`, `ch_detail_${ch.chatId}`),
  ]);
  buttons.push([backBtn('← Back', 'channels')]);

  const lines = pending.map(ch =>
...
bot.action(/^ch_detail_(-?\d+)$/, adminOnly, async ctx => {
  ...
  const info = [
    ...
    ch.resolvedAt ? `Resolved: ${new Date(ch.resolvedAt).toLocaleString('en-US', { dateStyle:'medium', timeStyle:'short' })}` : null,
  ].filter(Boolean).join('\n');

  const buttons = [];
  if (ch.status !== 'active')   buttons.push(Markup.button.callback('✅ Approve', `ch_approve_${ch.chatId}`));
  if (ch.status !== 'declined') buttons.push(Markup.button.callback('❌ Decline', `ch_decline_${ch.chatId}`));
...
bot.action(/^ch_approve_(-?\d+)$/, adminOnly, async ctx => {
  ...
  console.log(`✅  Channel approved: ${ch.title} (${chatId}) by ${ctx.from.id}`);

  await ctx.editMessageText(
    `✅ *${ch.title}* has been *approved*.\n\nThe bot will now track subscriber joins for this channel.`,
    { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[backBtn('← Back to Channels', 'channels_list')]]) }
  );
});
...
bot.action(/^ch_decline_(-?\d+)$/, adminOnly, async ctx => {
  ...
  console.log(`❌  Channel declined: ${ch.title} (${chatId}) by ${ctx.from.id}`);
  ...
  if (!wasAdmin && isNowAdmin) {
    ...
    if (ch.status === 'pending') {
      ...
      try {
        ...
        console.log(`📨  Notified admin ${BOT_ADMIN_ID} about new channel: ${chat.title}`);
      } catch (err) {
        console.error('⚠️  Could not DM admin for channel approval:', err.message);
      }
    }
  }
...
bot.on('chat_member', async ctx => {
  ...
  const isJoin = new_chat_member.status === 'member' &&
...
if (existing) {
  console.log(`↩️  Returning subscriber: ${user.first_name} (${user.id}) — skipping`);
  return;
}
...
console.log(`➕  New subscriber: ${user.first_name} (${user.id}) via channel ${update.chat.id}`);
...
bot.on('new_chat_members', async ctx => {
  ...
  for (const user of ctx.message.new_chat_members) {
    ...
    try {
      ...
      console.log(`➕  New member (group event): ${user.first_name} (${user.id})`);
      broadcast({ type: 'NEW_SUBSCRIBER', data: doc });
    } catch (err) {
...
bot.action('add_admin_prompt', adminOnly, async ctx => {
  ...
  await ctx.editMessageText(
    ...
    { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('✕ Cancel', 'admins')]]) }
  );
});
...
bot.on('text', adminOnly, async (ctx, next) => {
  ...
  if (state.action === 'add_admin') {
    ...
    if (await Admin.findOne({ telegramId: targetId })) {
      return ctx.reply(`⚠️ \`${targetId}\` is already an admin.`, {
        parse_mode: 'Markdown', ...Markup.inlineKeyboard([[backBtn('← Back to Admins','admins')]]),
      });
    }
    await Admin.create({ telegramId: targetId, addedBy: ctx.from.id, firstName: '' });
  ...
  if (state.action === 'remove_admin') {
    if (IS_MOCK) return ctx.reply('🎭 Mock mode — DB not available.');
    if (targetId === Number(BOT_ADMIN_ID)) return ctx.reply('⛔ Cannot remove bootstrap admin.');
